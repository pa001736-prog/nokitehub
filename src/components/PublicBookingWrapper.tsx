...
export default function PublicBookingWrapper() {
  ...
  const addAppointment = async (appointment: Appointment) => {
    ...
    try {
      ...
      // Ensure push notification is also fired
      try {
        ...
        const message = `Um novo agendamento foi realizado por ${appointment.clientName} no dia ${formattedDate} às ${appointment.time}.`;

        tokensSnap.forEach((doc) => {
          const tokenData = doc.data();
          if (tokenData.token) {
            sendPushNotification(tokenData.token, 'Novo Agendamento!', message, { link: '/admin' });
          }
        });
      } catch (navError) {
        console.error("Error sending notification across users:", navError);
      }
    } catch (error) {
