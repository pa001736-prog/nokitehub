...
export default function SuperAdminDashboard({ user }: SuperAdminDashboardProps) {
  ...
  const handlePlanChange = async (shopId: string, newPlan: Subscription['plan']) => {
    ...
    try {
      ...
      // Se o plano estiver vencido e trocarmos o plano, vamos renovar automaticamente por 30 dias
      if (shop?.subscription?.expiresAt && new Date(shop.subscription.expiresAt) <= new Date()) {
        const nextMonth = new Date();
        nextMonth.setDate(nextMonth.getDate() + 30);
        updates['subscription.expiresAt'] = nextMonth.toISOString();
  ...
  const handleRenewPlan = async (shopId: string, days: number) => {
    setIsUpdating(shopId);
    try {
      const shop = shops.find(s => s.id === shopId);
      const currentDate = shop?.subscription?.expiresAt && new Date(shop.subscription.expiresAt) > new Date() 
                          ...
                          <div className="flex flex-col">
                            ...
                            <div className="flex flex-col gap-1 mt-1">
                              ...
                              <div className="flex gap-1">
                                <button
                                  onClick={() => handleRenewPlan(shop.id, 7)}
                                  disabled={isUpdating === shop.id}
                                  className="text-[10px] bg-blue-50 text-blue-600 hover:bg-blue-100 px-2 py-0.5 rounded font-bold uppercase tracking-widest transition-colors"
                                >
                                  +7 dias
                                </button>
                                <button
                                  onClick={() => handleRenewPlan(shop.id, 30)}
                                  disabled={isUpdating === shop.id}
                                  className="text-[10px] bg-green-50 text-green-600 hover:bg-green-100 px-2 py-0.5 rounded font-bold uppercase tracking-widest transition-colors"
                                >
                                  +30 dias
                                </button>
                              </div>
