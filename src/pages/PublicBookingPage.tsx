...
import { Service, Settings, Appointment, PaymentMethod, BusySlot, Professional } from '../types';

type BookingStep = 'SERVICES' | 'CONFIRM_SERVICES' | 'PROFESSIONAL' | 'DATETIME' | 'PAYMENT' | 'DETAILS';
...
export default function PublicBookingPage({ shopId, settings, busySlots, professionals, onAddAppointment }: PublicBookingPageProps) {
  ...
  useEffect(() => {
    const localApps = localStorage.getItem(`nokite_appointments_${shopId}`);
    if (localApps) {
      try {
        setMyAppointments(JSON.parse(localApps));
      } catch (e) {
        console.error('Failed to parse local appointments');
      }
    }
  }, [shopId]);
  ...
  const handleSearchAppointments = async (e: React.FormEvent) => {
    ...
    // Filter local storage appointments by phone
    try {
      const localApps = localStorage.getItem(`nokite_appointments_${shopId}`);
      if (localApps) {
        const parsed = JSON.parse(localApps) as Appointment[];
        const filtered = parsed.filter(a => a.phone === searchPhone);
      ...
      toast.error('Erro ao buscar agendamentos locais.');
    } finally {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      const localApps = localStorage.getItem(`nokite_appointments_${shopId}`);
      const appsArray = localApps ? JSON.parse(localApps) : [];
      appsArray.push(appointment);
      localStorage.setItem(`nokite_appointments_${shopId}`, JSON.stringify(appsArray));
      
      // Update myappointments state if looking
      setMyAppointments(appsArray);
  ...
  const prevStep = () => {
    ...
    else if (currentStep === 'PROFESSIONAL') setCurrentStep('CONFIRM_SERVICES');
        ...
        {currentStep !== 'SERVICES' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center px-2">
              {['Serviços', 'Revisão', 'Profissional', 'Horário', 'Pagamento', 'Dados'].map((label, idx) => {
                const steps = ['SERVICES', 'CONFIRM_SERVICES', 'PROFESSIONAL', 'DATETIME', 'PAYMENT', 'DETAILS'];
                ...
                return (
                  <div key={label} className="flex flex-col items-center gap-3">
                    ...
                    <span className={`text-[9px] font-bold uppercase tracking-[0.2em] transition-colors duration-500 ${isActive ? 'text-[var(--accent)]' : 'text-transparent'} hidden sm:block`}>
                      {label}
                    </span>
                  </div>
                );
            ...
            <div className="h-[1px] bg-[var(--border)] relative">
              <motion.div 
                initial={false}
                animate={{ 
                  width: currentStep === 'CONFIRM_SERVICES' ? '20%' :
                         currentStep === 'PROFESSIONAL' ? '40%' :
                         currentStep === 'DATETIME' ? '60%' : 
        ...
        <div className="space-y-12">
          <AnimatePresence mode="wait">
            ...
            {currentStep === 'PROFESSIONAL' && (
              ...
              >
                <div className="space-y-2">
                  <h3 className="text-4xl font-sans font-black uppercase tracking-tight text-[var(--primary)]">Profissional</h3>
                  <p className="font-sans font-bold text-[var(--accent)] text-sm uppercase tracking-widest">Quem vai realizar o serviço?</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {professionals.length === 0 ? (
                    <div className="col-span-full p-8 border border-[var(--border)] bg-white text-center">
                      <p className="text-[var(--muted-foreground)] uppercase tracking-widest text-xs font-bold">Nenhum profissional disponível no momento.</p>
                    </div>
                    ...
                    <button
                      ...
                      className={`flex items-center gap-4 p-6 bg-white border transition-all text-left ${
                        selectedProfessional === prof.id
                          ? 'border-[var(--accent)] shadow-md ring-1 ring-[var(--accent)]'
                          : 'border-[var(--border)] hover:border-[var(--accent)]/50'
                      }`}
                    >
                      <div className="w-16 h-16 rounded-full overflow-hidden bg-[var(--muted)] border border-[var(--border)] flex-shrink-0">
                        {prof.photo ? (
                      ...
                      <div className="flex-1 space-y-1">
                        <p className="font-sans font-black text-xl text-[var(--primary)] tracking-tight">{prof.name}</p>
                        <p className="text-[10px] uppercase tracking-widest font-bold text-[var(--muted-foreground)]">{prof.role}</p>
                      </div>
                      <div className={`w-6 h-6 rounded-full border flex items-center justify-center transition-all duration-500 ${
                        selectedProfessional === prof.id ? 'bg-[var(--accent)] border-[var(--accent)]' : 'border-[var(--border)]'
                      }`}>
                        {selectedProfessional === prof.id && <Check className="w-3 h-3 text-white" />}
                      </div>
                    </button>
                  ))}
                </div>
                            ...
                            <div className="mt-4 pt-4 border-t border-slate-100 flex justify-end">
                              <button 
                                onClick={() => {
                                  const cleanPhone = settings?.shopPhone ? settings.shopPhone.replace(/\D/g, '') : '';
                                  if (cleanPhone) {
                                    const msg = `Olá! Preciso de ajuda com meu agendamento do dia ${new Date(app.date).toLocaleDateString('pt-BR')} às ${app.time}.`;
                                    window.open(`https://wa.me/55${cleanPhone}?text=${encodeURIComponent(msg)}`, '_blank');
                                  }
                                }}
